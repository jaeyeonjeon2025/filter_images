...
const activateLightBox = (thisElmt) => {
  ...
  const firstLetter = thisElmtDataFilter.charAt(0).toUpperCase(); // 첫번째 글자를 선택하여 대문자로 변환
  const remainLetters = thisElmtDataFilter.slice(1); // 두번째 글자부터 나머지 글자 전체를 선택
  ...
  // 함수가 실행되면 클릭된 모든 이미지 요소의 active 요소 삭제
  images.forEach((img) => img.classList.remove("active"));
  // 클릭한 대상 이미지 요소에 active 클래스 추가
  thisElmt.classList.add("active");

  // 라이트 박스가 활성화될 때 클릭된 이미지 요소의 src 속성값을 읽어 라이트 박스 이미지 src 속성값으로 전달
...
// 이미지 요소들을 순회하여 각각을 클릭했을 때 activeLightBox 함수 실행: 클릭한 이미지 요소를 파라미터로 전달
images.forEach((img) => {
...
// 이미지 요소 배열을 순회한다. 첫번째 파라미터는 이미지 요소 배열이고, 두번째 파라미터는 boolean 값 - true일 경우 '?' 뒤에 ':'을 기준으로 앞의 것이 적용되고, false일 경우 뒤의 것이 적용된다.
...
btns.forEach((btn) => {
  ...
  btn.addEventListener("click", function () {
    ...
    // 이미지 요소들이 재배치될 때 0.1초 뒤에 showItems 함수 실행
    setTimeout(() => {
      showItems(this);
    }, 100);
  });
...
//    const idAttr=document.querySelector('.light-box-image').getAttribute('id');
//    console.log(idAttr);
...
// setTimeout(function(){
//     alert('I AM NOT HELLO');
// }, 1000);

// setInterval(): https://ko.javascript.info/settimeout-setinterval#ref-204
// 변수에 값을 저장하게 되면, 변수를 이용하여 일정 시간 이후 실행을 정지시킬 수 있다. (clearInterval(timerId))
// const timerId = setInterval(() => console.log('째깍'), 2000);
// setTimeout(() => { clearInterval(timerId)}, 5000); // 5초 후에 정지

// // 상황연산자
// const a=1;
// a === 1 ? alert('correct') : alert('wrong!!!');
